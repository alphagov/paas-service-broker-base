	...
	BeforeEach(func() {
		...
		validConfig = Config{
			API: API{
				...
				Locket: LocketConfig{
					Address:        locketAddress,
					CACertFile:     path.Join(fixturesPath, "locket-server.cert.pem"),
					ClientCertFile: path.Join(fixturesPath, "locket-client.cert.pem"),
					ClientKeyFile:  path.Join(fixturesPath, "locket-client.key.pem"),
					SkipVerify:     true,
				},
			},
